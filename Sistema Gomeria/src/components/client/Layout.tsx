...
export function ClientApp({ auth }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--br-bg)' }}>
      {/* Header */}
      <header className="sticky top-0 z-40" style={{ background: 'var(--br-dark)', borderBottom: '1px solid var(--br-dark-bor)' }}>
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: 'var(--br-dark-txt2)' }}>{auth.clientName}</p>
            </div>
          </div>
          <button onClick={auth.logout} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors hover:bg-white/15" style={{ color: 'var(--br-dark-txt2)', background: 'rgba(255,255,255,0.08)' }}>
            <LogOut className="h-3.5 w-3.5" /> Salir
          </button>
        </div>

        {/* Tab nav */}
        <div className="max-w-4xl mx-auto px-4 flex" style={{ borderTop: '1px solid var(--br-dark-bor)' }}>
          {TABS.map(({ id, label, Icon }) => (
            <button key={id} onClick={() => setTab(id)}
              className="flex items-center gap-1.5 px-4 py-3 text-xs font-semibold transition-colors"
              style={{
                color: tab === id ? 'var(--br-amb)' : 'var(--br-dark-txt2)',
